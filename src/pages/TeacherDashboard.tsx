...
const TeacherDashboard: React.FC = () => {
  ...
  const handleCreateProject = (projectData: Omit<Project, 'id' | 'createdAt' | 'tasks' | 'teacherId' | 'teacherName'>) => {
    if (user) {
      createProject({
        ...projectData,
        teacherId: user.id,
  ...
  const handleCreateTask = (taskData: Omit<Task, 'id' | 'createdAt' | 'assignedToNames'>) => {
    if (selectedProject) {
      addTask(selectedProject.id, taskData);
      setShowCreateTask(false);
      setSelectedProject(null);
    }
  };

  const handleTaskStatusUpdate = (projectId: string, taskId: string, status: Task['status']) => {
